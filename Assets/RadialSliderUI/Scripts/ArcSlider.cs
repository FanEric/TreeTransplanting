			...
			{                    
				...
				if (isPointerDown)
				{
                    ...
                    //if (mouseRadius > ignoreInTouchRadiusHandleOffset)// && handleButton.GetComponent<RectTransform>().localPosition.x <= 0
                    {
                        ...
                        {
                            ringImage.fillAmount = tempangle;
                            lastAngle = tempangle;
                            //angle = (tempAngle + angle) / 2f;
                            //tempAngle = angle;
                            //改变小圆的位置
                            handleButton.GetComponent<RectTransform>().localPosition = new Vector3(Mathf.Cos(-angle / Mathf.Rad2Deg + 22.5f * Mathf.PI) * circleRadius, Mathf.Sin(-angle / Mathf.Rad2Deg + 22.5f * Mathf.PI) * circleRadius, 0);
